...
RAW_DATA_WORKSHEET_NAME = "Form Responses 1 (RAW)" 
...
def load_to_supabase(supabase: Client, table_name: str, df: pd.DataFrame, pk_column: str, is_log=False):
    ...
    try:
        if is_log:
            ids_to_update = df_cleaned[pk_column].unique().tolist()
            if ids_to_update:
                print(f"   - Menghapus log lama untuk {len(ids_to_update)} responden...")
                supabase.table(table_name).delete().in_(pk_column, ids_to_update).execute()
            print("   - Memasukkan log baru...")
...
def transform_all_data(df_raw):
        ...
        for hari_name, keg_cols in hari_map_kegiatan.items():
            for i, keg_col in enumerate(keg_cols):
                ...
                if kegiatan and str(kegiatan).strip() and str(kegiatan).lower() != "tidak di kampus":
                    ...
                    if isinstance(lokasi_raw, str) and lokasi_raw.strip() not in ['-', '']: lokasi = random.choice([loc.strip() for loc in lokasi_raw.split(',')])
                    is_facility_intensive = 'Kelas' in kegiatan or 'Makan/Minum' in kegiatan

                    emisi_ac_val = 0.83 if is_facility_intensive else 0
    ...
    def clean_duration(text): return int(re.findall(r'\d+', str(text))[0]) if re.findall(r'\d+', str(text)) else 0
    ...
    jumlah_hari_datang = df_elektronik['hari_datang'].apply(lambda x: len([h.strip() for h in str(x).split(',') if h.strip()]) if pd.notna(x) and str(x).strip() else 0)
    
    emisi_pribadi_harian = ((df_elektronik['durasi_hp'] * 4) + (df_elektronik['durasi_laptop'] * 50) + (df_elektronik['durasi_tab'] * 10)) * 0.829 / 1000 / 60
    df_elektronik['emisi_elektronik_pribadi'] = emisi_pribadi_harian * jumlah_hari_datang
