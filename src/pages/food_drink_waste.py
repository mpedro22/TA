...
def generate_pdf_report(where_clause, join_needed):
    ...
    total_emisi = daily_stats['total_emisi'].sum() if not daily_stats.empty else 0
    if total_emisi == 0:
        return "<html><body><h1>Tidak ada data untuk dilaporkan</h1><p>Silakan ubah filter Anda.</p></body></html>"

    total_activities = daily_stats['activity_count'].sum() if not daily_stats.empty else 0
    avg_emisi_per_activity = total_emisi / total_activities if total_activities > 0 else 0

    daily_trend_table_html, trend_conclusion, trend_recommendation = ("<tr><td colspan='3'>N/A</td></tr>", "Pola emisi harian tidak dapat diidentifikasi.", "Data tidak cukup untuk analisis tren.")
    ...
    period_table_html, period_conclusion, period_recommendation = ("<tr><td colspan='3'>N/A</td></tr>", "Distribusi periode makan tidak dapat dianalisis.", "Data tidak cukup untuk analisis periode.")
    heatmap_header_html, heatmap_body_html, heatmap_conclusion, heatmap_recommendation = ("<tr><th>-</th></tr>", "<tr><td>N/A</td></tr>", "Pola emisi antar lokasi dan waktu belum dapat dipetakan.", "Data tidak cukup untuk mengidentifikasi hotspot.")
    canteen_table_html, canteen_conclusion, canteen_recommendation = ("<tr><td colspan='3'>N/A</td></tr>", "Lokasi dengan kontribusi emisi terbesar tidak dapat ditentukan.", "Data tidak cukup untuk analisis lokasi.")
    if not daily_stats.empty:
        ...
        if len(daily_stats) > 1:
            peak_day = daily_stats.loc[daily_stats['total_emisi'].idxmax()]
            trend_conclusion = f"Puncak emisi sampah makanan terjadi pada hari <strong>{peak_day['hari']}</strong>, dengan total <strong>{peak_day['total_emisi']:.1f} kg CO₂</strong> dari {peak_day['activity_count']} aktivitas. Ini mengindikasikan pola konsumsi mingguan yang jelas."
            ...
            day_name = daily_stats.iloc[0]['hari']
            trend_conclusion = f"Data yang ditampilkan hanya untuk hari <strong>{day_name}</strong>. Analisis tren memerlukan perbandingan antar hari."
            trend_recommendation = "Perluas rentang hari pada filter untuk melihat pola mingguan. Untuk hari ini, fokus analisis bisa dialihkan ke distribusi per periode waktu atau per lokasi."
    ...
    if not period_stats.empty:
        ...
        period_conclusion = f"Periode <strong>{peak_period['meal_period']}</strong> menjadi 'critical window' dengan kontribusi emisi tertinggi, yaitu <strong>{peak_period['total_emisi']:.1f} kg CO₂</strong>."
    ...
    if not heatmap_data.empty:
        ...
        if not pivot_df.empty:
            ...
            heatmap_header_html = "<tr><th>Lokasi</th>" + "".join([f"<th>{time}</th>" for time in pivot_df.columns]) + "</tr>"
            body_rows_list = [f"<tr><td><strong>{loc}</strong></td>" + "".join([f"<td style='text-align:center;'>{val:.2f}</td>" for val in row]) + "</tr>" for loc, row in pivot_df.iterrows()]
            heatmap_body_html = "".join(body_rows_list)
            hotspot_value = pivot_df.max().max()
            if hotspot_value > 0:
                hotspot_time = pivot_df.max().idxmax()
                hotspot_location = pivot_df.idxmax()[hotspot_time]
                heatmap_conclusion = f"Teridentifikasi 'hotspot' emisi di <strong>{hotspot_location}</strong> pada jam <strong>{hotspot_time}</strong>, yang merupakan kombinasi lokasi dan waktu dengan emisi tertinggi."
    ...
    if not canteen_stats.empty:
        canteen_table_html = "".join([f"<tr><td>{row['lokasi']}</td><td style='text-align:right;'>{row['total_emisi']:.1f}</td><td style='text-align:center;'>{row['activity_count']}</td></tr>" for _, row in canteen_stats.iterrows()])
        hottest_canteen = canteen_stats.iloc[0]
        canteen_conclusion = f"Lokasi/kantin <strong>{hottest_canteen['lokasi']}</strong> adalah kontributor tunggal terbesar terhadap emisi sampah makanan, dengan rata-rata limbah per aktivitas sebesar <strong>{hottest_canteen['avg_emisi']:.2f} kg CO₂</strong>. Ini menunjukkan adanya masalah sistemik di lokasi ini."
        canteen_recommendation = f"Jadikan <strong>{hottest_canteen['lokasi']}</strong> sebagai lokasi percontohan program reduksi sampah. Langkah yang bisa diambil: (1) Mengadakan diskusi dengan vendor untuk evaluasi porsi. (2) Memfasilitasi sistem pemilahan sampah organik yang lebih baik di lokasi ini."
    html_content = f"""
    <!DOCTYPE html><html><head><title>Laporan Emisi Sampah Makanan</title><link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600&display=swap" rel="stylesheet"><style>body{{font-family:'Poppins',sans-serif;color:#333;line-height:1.6;font-size:11px}}.page{{padding:25px;max-width:800px;margin:auto}}.header{{text-align:center;border-bottom:2px solid #059669;padding-bottom:15px;margin-bottom:25px}}h1{{color:#059669;margin:0}}h2{{color:#065f46;border-bottom:1px solid #d1fae5;padding-bottom:8px;margin-top:30px;margin-bottom:15px}}.grid{{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:25px}}.card{{background:#f8f9fa;padding:15px;border-radius:8px;text-align:center}}.card.primary{{border-left:4px solid #10b981}}.card.primary strong{{color:#059669}}.card.secondary{{border-left:4px solid #3b82f6}}.card.secondary strong{{color:#3b82f6}}.card strong{{font-size:1.5em;display:block}}.conclusion,.recommendation{{padding:12px 15px;margin-top:10px;border-radius:6px}}.conclusion{{background:#f0fdf4;border-left:4px solid #10b981}}.recommendation{{background:#fffbeb;border-left:4px solid #f59e0b}}table{{width:100%;border-collapse:collapse;margin-top:15px}}th,td{{padding:8px;text-align:left;border:1px solid #e5e7eb}}th{{background-color:#f3f4f6;font-weight:600;text-align:center}}td:first-child{{font-weight:500}}</style></head>
    <body><div class="page">
        ...
        <div class="grid">
            <div class="card primary"><strong>{total_emisi:.1f} kg CO₂</strong>Total Emisi (sesuai filter)</div>
            <div class="card secondary"><strong>{avg_emisi_per_activity:.2f} kg CO₂</strong>Rata-rata/Aktivitas</div>
        </div>
        <h2>1. Tren Emisi Harian</h2>
        <table><thead><tr><th>Hari</th><th>Total Emisi (kg CO₂)</th><th>Jumlah Aktivitas</th></tr></thead><tbody>{daily_trend_table_html}</tbody></table>
        ...
        <table><thead><tr><th>Periode</th><th>Total Emisi (kg CO₂)</th><th>Jumlah Aktivitas</th></tr></thead><tbody>{period_table_html}</tbody></table>
...
def show():
    st.markdown("""
        <div class="wow-header">
            ...
            <div class="header-content"><h1 class="header-title">Emisi Sampah Makanan & Minuman</h1></div>
        </div>
    ...
    with export_col2:
        try:
            st.download_button("Laporan", data=generate_pdf_report(where_clause, join_needed), file_name=f"food_waste_report.html", mime="text/html", use_container_width=True, key="food_export_pdf")
